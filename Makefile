...
build:
	...
	@ORIGINAL_VERSION=$$(grep '^__version__ = ' src/neat_resume/__about__.py | cut -d'"' -f2); \
	GIT_TAG=$$(git describe --tags --exact-match 2>/dev/null || echo ""); \
	if [ -n "$$GIT_TAG" ] && echo "$$GIT_TAG" | grep -E "^[0-9]+\.[0-9]+\.[0-9]+$$" > /dev/null; then \
		echo "Using semantic version tag: $$GIT_TAG"; \
		sed -i "s/^__version__ = .*/__version__ = \"$$GIT_TAG\"/" src/neat_resume/__about__.py; \
	else \
		SHORT_SHA=$$(git rev-parse --short HEAD); \
		SHA_INT=$$(printf "%d" 0x$$SHORT_SHA); \
		echo "Using SHA-based version: $$ORIGINAL_VERSION-$$SHA_INT"; \
		sed -i "s/^__version__ = .*/__version__ = \"$$ORIGINAL_VERSION-$$SHA_INT\"/" src/neat_resume/__about__.py; \
	fi; \
	trap 'git checkout -- src/neat_resume/__about__.py' EXIT; \
	hatch clean; \
	hatch build; \
...
clean:
	...
	git checkout -- src/neat_resume/__about__.py
