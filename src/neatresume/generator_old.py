...
from reportlab.lib.colors import black, darkgray, HexColor
...
from neatresume.resume import Resume, CandidateInfo, EducationBlock, CertificationBlock
...
class TintedPageTemplate(pydantic.BaseModel, PageTemplate):
    """Custom PageTemplate with a tinted background for the left column."""

    def __init__(self, left_col_width: float, page_width: float, page_height: float, left_margin: float, **kwargs: Any) -> None:
    ...
    def beforeDrawPage(self, canvas: Canvas, doc: Any) -> None:
...
class ResumeGenerator:
    ...
    def _setup_custom_styles(self) -> None:
        ...
        self.styles.add(
            ParagraphStyle(name="LeftColumnHeader", parent=self.styles["SectionHeader"], fontSize=9, spaceBefore=8, spaceAfter=0)
        )
